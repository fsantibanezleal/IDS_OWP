function v_LocSamples_New = LocateSamples_Oracle_MES(strParamS)
%% Inputs
...
im_Samp = entropyfilt(im_R,ones(3));
im_Samp = im_Samp  - min(im_Samp(:));
im_Samp = im_Samp ./ max(im_Samp(:));
im_Samp = im_Samp >= 0.3;

idx_Free = find(im_Samp > 0);
