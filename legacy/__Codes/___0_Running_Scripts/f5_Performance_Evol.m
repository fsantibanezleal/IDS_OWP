function varargout = f5_Performance_Evol(varargin)
...
if validSelection    
    ...
    if (numel(strfind(filesSamp(idxSP).name, 'Sampling_Process_')) > 0) &&...
       (numel(strfind(filesSamp(idxSP).name, '.mat')) > 0) && ...
        ...
        for idxIterS = [1,(20:20:180),nSamples]
            % 1 -> Random Uniform
            % Provide Path to oputput Folder
...
dataSS_Point.evoEnt.Y = mean2(Hsims);
dataSS_Point.evoEnt.EL = std2(Hsims - mean2(Hsims));%abs( mean(Hsims(:)) - min(Hsims(:)));%std2(Hsims);
dataSS_Point.evoEnt.EU = std2(Hsims - mean2(Hsims));%abs( mean(Hsims(:)) - max(Hsims(:)));%std2(Hsims);
...
dummySTR = ['''hSS'  num2str(idxAxis) '_Samples'''];
eval(['SaveAxesNow(' 'handles.hSS' num2str(idxAxis) '_Samples' ',' dummySTR ');' ]);
dummySTR = ['''hSS'  num2str(idxAxis) '_Ent'''];
eval(['SaveAxesNow(' 'handles.hSS' num2str(idxAxis) '_Ent' ','    dummySTR   ');' ]);
dummySTR = ['''hSS'  num2str(idxAxis) '_Mean'''];
eval(['SaveAxesNow(' 'handles.hSS' num2str(idxAxis) '_Mean' ','   dummySTR  ');' ]);
dummySTR = ['''hSS'  num2str(idxAxis) '_Std'''];
eval(['SaveAxesNow(' 'handles.hSS' num2str(idxAxis) '_Std' ','    dummySTR   ');' ]);
